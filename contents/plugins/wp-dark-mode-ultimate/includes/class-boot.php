...
if ( ! class_exists( __NAMESPACE__ . '\Boot' ) ) {
	...
	class Boot {
		...
		public function load_admin_files() {
			...
			require_once WP_DARK_MODE_ULTIMATE_INCLUDES . '/admin/class-admin-analytics.php';
		...
		public function init_appsero() {
			...
			// Load Appsero if not loaded.
			if ( ! class_exists( '\WP_Dark_Mode\Appsero\Client' ) ) {
				require_once WP_DARK_MODE_ULTIMATE_INCLUDES . '/appsero/Client.php';
			}

			// Load Updater.
			require_once WP_DARK_MODE_ULTIMATE_INCLUDES . '/appsero/Updater.php';

			$client = new \WP_Dark_Mode\Appsero\Client( '44e81435-c0f1-4149-983b-eb8d9f7a9a66', 'WP Dark Mode Ultimate', WP_DARK_MODE_ULTIMATE_FILE );

			// Active insights.
			$client->insights()->hide_notice()->init();

			// Initialize updater.
			\WP_Dark_Mode\Appsero\Updater::init($client);
			...
			$args = [
				'type'        => 'submenu',
				'menu_title'  => __('License Activation', 'wp-dark-mode-ultimate'),
				'page_title'  => __('License Activation - WP Dark Mode', 'wp-dark-mode-ultimate'),
