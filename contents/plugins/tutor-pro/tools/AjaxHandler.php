...
class AjaxHandler {
	...
	public function ajax_import_handler() {
		...
		if ( is_wp_error( $contents ) ) {
			$this->response_bad_request( $contents->get_error_message() );
		}
		...
		if ( $contents ) {
			...
			$result = $this->prepare_import_job_contents( $data );

			if ( is_wp_error( $result ) ) {
				$this->json_response( __( 'Error preparing import job', 'tutor-pro' ), $result->get_error_message(), HttpHelper::STATUS_BAD_REQUEST );
			}
					...
					case $this->importer::TYPE_CONTENT_BANK:
						if ( $ids && tutor_utils()->is_addon_enabled( 'content-bank' ) ) {
							...
							foreach ( $content['data'] as $data ) {
								...
								$this->send_import_response( __( 'Importing Content Bank', 'tutor-pro' ) );
							}
						}
						break;
	 ...
	 */
	private function prepare_import_job_contents( $data ) {
