...
async function openProgress() {
    ...
    try {
        ...
        if (result.success && result.found) {
            Continue(result.data.progress);
            
        } else {
            modalContent.innerHTML = `
                <div class="no-progress">
                    <p>Không có bản ghi cho test này</p>
                    <button onclick="closeModal()">Close</button>
                </div>
            `;
            
            // Chuyển hướng sau 2 giây (tuỳ bạn chỉnh)
            setTimeout(() => {
                window.location.href = `${siteUrl}/practice/dictation/${id_test}`;
            }, 2000);
        }
