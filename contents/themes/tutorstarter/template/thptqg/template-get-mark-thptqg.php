...
if ($result_test->num_rows > 0) {
    ...
    if (!empty($results)) {
        ...
        foreach ($results as $result) {
            ...
            echo '<a class="button-main" onclick="redoWrongAns()">Làm lại các câu sai</a><br><br>';
            ...
            foreach ($correctAnswers as $correct) {
                // Kiểm tra xem có đủ key cần thiết không
                if (isset($correct['part'], $correct['question'], $correct['answer'])) {
                    $part = $correct['part'];
                    $questionNumber = $correct['question'];
                    $correctAnswersMap[$part][$questionNumber] = $correct;
                }
            }

            // Process user answers by part
            foreach ($userAnswers as $part => $questions) {
                echo '<h3>PART ' . esc_html($part) . '</h3>';
                echo '<div class = "result-answers-list">';
                
                foreach ($questions as $question) {
                    $questionId = $question['id'];
                    $userAnswer = $question['answer'];
                    $questionNumber = substr($questionId, strrpos($questionId, '_') + 1);
            
                    // Lấy đáp án đúng từ $correctAnswersMap
                    $correctData = $correctAnswersMap[$part][$questionNumber] ?? null;
                    $correctAnswer = $correctData['answer'] ?? 'N/A'; // Nếu không tìm thấy, gán 'N/A'
                        ...
                        switch ($questionType) {
                            ...
                            case 'completion':
                                if (is_array($correctAnswer)) {
                                    ...
                                    $isCorrect = true;
                                    foreach ($correctAnswer as $i => $correctPart) {
                                        if (!isset($userParts[$i])) {
                                            $isCorrect = false;
                                            break;
                                        }
                                        if (strtolower(trim($userParts[$i])) !== strtolower(trim($correctPart))) {
                                            $isCorrect = false;
                                            break;
                                        }
                                    }
                            ...
                            case 'true-false':
                                ...
                                $userParts = explode('/', $userAnswer);
                                $isCorrect = true;
                                foreach ($correctParts as $i => $correctPart) {
                                    if (!isset($userParts[$i])) {
                                        $isCorrect = false;
                                        break;
                                    }
                                    if ($userParts[$i] !== '...' && $userParts[$i] !== $correctPart) {
                                        $isCorrect = false;
                                        break;
                                    }
                                }
                    ...
                    <div class="result-answers-item">
                        <div class="question-row">
                            <span class="question-number"><strong>' . esc_html($part . '.' . $questionNumber) . '</strong></span>
