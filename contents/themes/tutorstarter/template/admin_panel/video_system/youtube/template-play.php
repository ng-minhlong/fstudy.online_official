...
if ($video_id) {
    ...
    $course_id = $wpdb->get_var($wpdb->prepare("SELECT course_id FROM $table_name WHERE youtube_id = %s", $video_id));
...
<body>
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', async () => {
      ...
      const resp = await fetch(`${site_url}/api/v1/video-data/${VIDEO_ID}`);
