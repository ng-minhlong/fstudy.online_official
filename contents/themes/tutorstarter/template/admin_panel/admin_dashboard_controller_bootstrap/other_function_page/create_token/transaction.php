...
$order_code_filter = isset($_GET['order_code_filter']) ? $_GET['order_code_filter'] : '';
...
$total_sql = "SELECT COUNT(*) FROM token_transaction";
if ($order_code_filter) {
    $total_sql .= " WHERE order_code LIKE '%$order_code_filter%'"; // Apply filter to total count
}
$total_result = $conn->query($total_sql);
$total_rows = $total_result->fetch_row()[0];
$total_pages = ceil($total_rows / $limit); // Calculate total pages

$sql = "SELECT * FROM token_transaction";
if ($order_code_filter) {
    $sql .= " WHERE order_code LIKE '%$order_code_filter%'"; // Apply filter to the SQL query
}
...
<head>
    ...
    <meta charset="UTF-8">
    <title>Token Transaction Admin System</title>
...
<form method="GET" action="">
        <div class="mb-3">
            <label for="order_code_filter" class="form-label">Filter by Order Code:</label>
            <input type="text" name="order_code_filter" id="order_code_filter" class="form-control" value="<?php echo isset($_GET['order_code_filter']) ? $_GET['order_code_filter'] : ''; ?>">
        </div>
...
<table class="table table-bordered">
  ...
  <nav aria-label="Page navigation">
        <ul class="pagination justify-content-center">
            <?php if ($page > 1): ?>
                <li class="page-item"><a class="page-link" href="?page=<?php echo $page - 1; ?>&order_code_filter=<?php echo $order_code_filter; ?>">Previous</a></li>
            <?php endif; ?>

            <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                <li class="page-item <?php if ($i == $page) echo 'active'; ?>">
                    <a class="page-link" href="?page=<?php echo $i; ?>&order_code_filter=<?php echo $order_code_filter; ?>"><?php echo $i; ?></a>
                </li>
            <?php endfor; ?>

            <?php if ($page < $total_pages): ?>
                <li class="page-item"><a class="page-link" href="?page=<?php echo $page + 1; ?>&order_code_filter=<?php echo $order_code_filter; ?>">Next</a></li>
            <?php endif; ?>
        </ul>
    </nav>
