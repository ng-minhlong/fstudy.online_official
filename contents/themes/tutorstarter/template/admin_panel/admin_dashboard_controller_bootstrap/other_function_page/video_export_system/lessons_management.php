...
$course_id_filter = isset($_GET['course_id_filter']) ? $_GET['course_id_filter'] : '';
...
$total_sql = "SELECT COUNT(*) FROM lessons_management";
if ($course_id_filter) {
    $total_sql .= " WHERE course_id LIKE '%$course_id_filter%'"; // Apply filter to total count
}
$total_result = $conn->query($total_sql);
$total_rows = $total_result->fetch_row()[0];
$total_pages = ceil($total_rows / $limit); // Calculate total pages

$sql = "SELECT * FROM lessons_management";
if ($course_id_filter) {
    $sql .= " WHERE course_id LIKE '%$course_id_filter%'"; // Apply filter to the SQL query
}
...
<form method="GET" action="">
        <div class="mb-3">
            <label for="course_id_filter" class="form-label">Filter by ID Test:</label>
            <input type="text" name="course_id_filter" id="course_id_filter" class="form-control" value="<?php echo isset($_GET['course_id_filter']) ? $_GET['course_id_filter'] : ''; ?>">
        </div>
...
<table class="table table-bordered">
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                echo "<tr id='row_{$row['number']}'>
                        ...
                        <td>{$row['youtube_status']}</td>
                        <td>{$row['abyss_slug']}</td>
  ...
  <nav aria-label="Page navigation">
        <ul class="pagination justify-content-center">
            <?php if ($page > 1): ?>
                <li class="page-item"><a class="page-link" href="?page=<?php echo $page - 1; ?>&course_id_filter=<?php echo $course_id_filter; ?>">Previous</a></li>
            <?php endif; ?>

            <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                <li class="page-item <?php if ($i == $page) echo 'active'; ?>">
                    <a class="page-link" href="?page=<?php echo $i; ?>&course_id_filter=<?php echo $course_id_filter; ?>"><?php echo $i; ?></a>
                </li>
            <?php endfor; ?>

            <?php if ($page < $total_pages): ?>
                <li class="page-item"><a class="page-link" href="?page=<?php echo $page + 1; ?>&course_id_filter=<?php echo $course_id_filter; ?>">Next</a></li>
            <?php endif; ?>
        </ul>
    </nav>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editForm">
                    ...
                    youtube_id <input type="text" id="edit_youtube_id" name="youtube_id" class="form-control" required><br>
                    youtube_status <input type="text" id="edit_youtube_status" name="youtube_status" class="form-control" required><br>
                    abyss_slug <input type="text" id="edit_abyss_slug" name="abyss_slug" class="form-control" required><br>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addForm">
                    ...
                    youtube_id <input type="text" id="add_youtube_id" name="youtube_id" class="form-control" required><br>
                    youtube_status <input type="text" id="add_youtube_status" name="youtube_status" class="form-control" required><br>
                    abyss_slug <input type="text" id="add_abyss_slug" name="abyss_slug" class="form-control" required><br>
...
function openEditModal(number) {
    $.ajax({
        ...
        success: function(response) {
            ...
            $('#edit_youtube_id').val(data.youtube_id);
            $('#edit_youtube_status').val(data.youtube_status);
            $('#edit_abyss_slug').val(data.abyss_slug);
            $('#edit_abyss_status').val(data.abyss_status);
            $('#edit_bunny_slug').val(data.bunny_slug);
            $('#edit_bunny_status').val(data.bunny_status);
