<?php
/*
 * Template Name: Test LIST Ielts Listening
 */
...
$total_sql = "SELECT COUNT(*) FROM topik_listening_test_list";
if ($id_test_filter) {
...
$sql = "SELECT * FROM topik_listening_test_list";
if ($id_test_filter) {
...
<head>
    ...
    <meta charset="UTF-8">
    <title>List các đề Topik Listening</title>
...
<h1>List các đề TOPIK Listening Test</h1>
</b>
            ...
            while($row = $result->fetch_assoc()) {
                ...
                echo "<tr id='row_{$row['number']}'>
                        ...
                        <td>
                                ...
                                <a href='http://localhost/test/topik/l/{$row['id_test']}' target='_blank'> {$row['id_test']}</a> 
...
function openEditModal(number) {
    $.ajax({
        url: '<?php echo get_site_url()?>/contents/themes/tutorstarter/template/topik/topik_listening/test-list/get_question.php', // Fetch the question details
        type: 'POST',
        data: { number: number },
...
function saveEdit() {
    $.ajax({
        url: '<?php echo get_site_url()?>/contents/themes/tutorstarter/template/topik/topik_listening/test-list/update_question.php',
        type: 'POST',
...
function saveNew() {
    $.ajax({
        url: '<?php echo get_site_url()?>/contents/themes/tutorstarter/template/topik/topik_listening/test-list/add_question.php',
        type: 'POST',
...
function deleteRecord(number) {
    if (confirm('Are you sure you want to delete this test ?')) {
        $.ajax({
            url: '<?php echo get_site_url()?>/contents/themes/tutorstarter/template/topik/topik_listening/test-list/delete_question.php',
            type: 'POST',
            data: { number: number },
            success: function(response) {
