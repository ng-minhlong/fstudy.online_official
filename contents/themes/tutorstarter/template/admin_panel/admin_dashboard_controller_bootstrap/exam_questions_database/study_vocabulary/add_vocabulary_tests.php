...
// Pagination logic
$limit = 60; // Number of records per page
$page = isset($_GET['page']) ? $_GET['page'] : 1; // Current page number
$offset = ($page - 1) * $limit; // Calculate offset

$total_sql = "SELECT COUNT(*) FROM list_test_vocabulary_book";
if ($id_test_filter) {
...
$sql = "SELECT * FROM list_test_vocabulary_book";
if ($id_test_filter) {
...
<head>
    ...
    <meta charset="UTF-8">
    <title>List Test Vocabulary</title>
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                echo "<tr id='row_{$row['number']}'>
                        <td>{$row['number']}</td>
                        <td>
                            <a href='http://localhost/practice/vocabulary/{$row['id_test']}' target='_blank'> {$row['id_test']}</a> 
                        </td>
                        <td>{$row['testname']}</td>
                        <td>{$row['test_type']}</td>
...
function openEditModal(number) {
    $.ajax({
        url: '<?php echo get_site_url()?>/contents/themes/tutorstarter/template/studyvocabulary/test-list/get_question.php', // Fetch the question details
        type: 'POST',
        data: { number: number },
...
function saveEdit() {
    $.ajax({
        url: '<?php echo get_site_url()?>/contents/themes/tutorstarter/template/studyvocabulary/test-list/update_question.php',
        type: 'POST',
...
function saveNew() {
    $.ajax({
        url: '<?php echo get_site_url()?>/contents/themes/tutorstarter/template/studyvocabulary/test-list/add_question.php',
        type: 'POST',
...
function deleteRecord(number) {
    if (confirm('Are you sure you want to delete this question?')) {
        $.ajax({
            url: '<?php echo get_site_url()?>/contents/themes/tutorstarter/template/studyvocabulary/test-list/delete_question.php',
            type: 'POST',
            data: { number: number },
            success: function(response) {
