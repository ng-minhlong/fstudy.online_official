...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<tr id='row_{$row['number']}'>
                <td>{$row['number']}</td>
                <td>
                    <a href='http://localhost/practice/talk-with-edward/{$row['id_test']}' target='_blank'> {$row['id_test']}</a>  
                </td>
                <td>{$row['testname']}</td>
                <td>{$row['target_1']}</td>
                <td>{$row['target_2']}</td>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editForm">
                    ...
                    Select Topic:
                    <select id="edit_topic" name="topic" class="form-control" required>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addForm">
                    ...
                    Select Topic:
                    <select id="add_topic" name="topic" class="form-control" required>
                        <option value="Shopping">Shopping</option>
                        <option value="Study">Study</option>
                        <option value="Art">Art</option>
                        <option value="Work">Work</option>
                    ...
                    Select Language:
                    <select id="add_language" name="language" class="form-control" required>
                        <option value="English">English</option>
                        <option value="French">French</option>
                        <option value="Korean">Korean</option>
                        <option value="Japanese">Japanese</option>
...
function openEditModal(number) {
    $.ajax({
        url: '<?php echo get_site_url()?>/contents/themes/tutorstarter/template/conversation_ai/list-topic-role-database/get_question.php', // Fetch the question details
        type: 'POST',
        data: { number: number },
...
function saveEdit() {
    $.ajax({
        url: '<?php echo get_site_url()?>/contents/themes/tutorstarter/template/conversation_ai/list-topic-role-database/update_question.php',
        type: 'POST',
...
function saveNew() {
    $.ajax({
        url: '<?php echo get_site_url()?>/contents/themes/tutorstarter/template/conversation_ai/list-topic-role-database/add_question.php',
        type: 'POST',
...
function deleteRecord(number) {
    if (confirm('Are you sure you want to delete this question?')) {
        $.ajax({
            url: '<?php echo get_site_url()?>/contents/themes/tutorstarter/template/conversation_ai/list-topic-role-database/delete_question.php',
            type: 'POST',
            data: { number: number },
            success: function(response) {
