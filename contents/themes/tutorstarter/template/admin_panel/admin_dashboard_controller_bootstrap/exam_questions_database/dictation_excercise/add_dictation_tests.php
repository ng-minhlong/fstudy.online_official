        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                echo "<tr id='row_{$row['number']}'>
                        <td>{$row['number']}</td>
                        <td>
                            <a href='http://localhost/dictation/{$row['id_test']}' target='_blank'> {$row['id_test']}</a> 
                        </td>
                        <td>{$row['type_test']}</td>
                        <td>{$row['testname']}</td>
                        <td>{$row['id_video']}</td>
...
function openEditModal(number) {
    $.ajax({
        url: 'http://localhost/contents/themes/tutorstarter/template/dictationexercise/dictation-exercise-template/get_question.php', // Fetch the question details
        type: 'POST',
        data: { number: number },
...
function saveEdit() {
    $.ajax({
        url: 'http://localhost/contents/themes/tutorstarter/template/dictationexercise/dictation-exercise-template/update_question.php',
        type: 'POST',
...
function saveNew() {
    $.ajax({
        url: 'http://localhost/contents/themes/tutorstarter/template/dictationexercise/dictation-exercise-template/add_question.php',
        type: 'POST',
...
function deleteRecord(number) {
    if (confirm('Are you sure you want to delete this question?')) {
        $.ajax({
            url: 'http://localhost/contents/themes/tutorstarter/template/dictationexercise/dictation-exercise-template/delete_question.php',
            type: 'POST',
            data: { number: number },
            success: function(response) {
