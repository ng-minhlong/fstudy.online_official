<?php
/*
 * Template Name: All Ielts Writing Results
 */
...
$total_sql = "SELECT COUNT(*) FROM save_user_result_ielts_writing";
if ($idtest_filter) {
...
$sql = "SELECT * FROM save_user_result_ielts_writing";
if ($idtest_filter) {
...
$sql2 = "SELECT * FROM ielts_writing_test_list";
...
<head>
    ...
    <meta charset="UTF-8">
    <title>All Ielts Writing Results</title>
    ...
    <?php include('../../sidebar.php'); ?>
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <?php include('../../topbar.php'); ?>
                ...
                <h1>All Ielts Writing Results</h1>
                        ...
                        while ($row = $result->fetch_assoc()) {
                            if ($row['test_type'] == 'Full Test') {
                                echo "<tr id='row_{$row['number']}'>
                                        ...
                                        <td>{$row['band_score']}</td>
                                        <td>{$row['band_score_expand']}</td>
                                        <td>{$row['timeSpent']}</td>
                                        <td>
    ...
    <table class="table table-bordered mt-3">
        ...
        if ($result_tests->num_rows > 0) {
            ...
            while ($row = $result_tests->fetch_assoc()) {
                ...
                $stats_sql = "SELECT 
                                AVG(band_score) AS avg_point, 
                                COUNT(*) AS test_times 
                              FROM save_user_result_ielts_writing 
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editForm">
                    ...
                    band_score: <input type="text" id="edit_band_score" name="band_score" class="form-control" required><br>
                    band_score_expand: <input type="text" id="edit_band_score_expand" name="band_score_expand" class="form-control" required><br>
                    timeSpent: <input type="text" id="edit_timeSpent" name="timeSpent" class="form-control" required><br>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="addForm">
                    ...
                    band_score: <input type="text" id="add_band_score" name="band_score" class="form-control" required><br>
                    band_score_expand: <input type="text" id="add_band_score_expand" name="band_score_expand" class="form-control" required><br>
                    timeSpent: <input type="text" id="add_timeSpent" name="timeSpent" class="form-control" required><br>
                </form>
            </div>
            <div class="modal-footer">
...
function openEditModal(number) {
    $.ajax({
        url: 'http://localhost/wordpress/contents/themes/tutorstarter/template/admin_panel/result_admin_page/ielts_writing_results/get_result.php', // Fetch the question details
        type: 'POST',
        data: { number: number },
        success: function(response) {
            ...
            $('#edit_band_score').val(data.band_score);
            $('#edit_band_score_expand').val(data.band_score_expand);
            $('#edit_timeSpent').val(data.timeSpent);
...
function saveEdit() {
    $.ajax({
        url: 'http://localhost/wordpress/contents/themes/tutorstarter/template/admin_panel/result_admin_page/ielts_writing_results/update_result.php',
        type: 'POST',
...
function openDetailsModal(idTest) {
    $.ajax({
        url: 'http://localhost/wordpress/contents/themes/tutorstarter/template/admin_panel/result_admin_page/ielts_writing_results/get_test_details.php',
        method: 'POST',
        data: { id_test: idTest },
        success: function(response) {
            // Đổ dữ liệu vào bảng
...
function saveNew() {
    $.ajax({
        url: 'http://localhost/wordpress/contents/themes/tutorstarter/template/admin_panel/result_admin_page/ielts_writing_results/add_result.php',
        type: 'POST',
...
function deleteRecord(number) {
    if (confirm('Are you sure you want to delete this result?')) {
        $.ajax({
            url: 'http://localhost/wordpress/contents/themes/tutorstarter/template/admin_panel/result_admin_page/ielts_writing_results/delete_result.php',
            type: 'POST',
            data: { number: number },
            success: function(response) {
