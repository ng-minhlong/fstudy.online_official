...
$total_sql = "SELECT COUNT(*) FROM ielts_listening_test_list";
if ($id_test_filter) {
...
$sql = "SELECT * FROM ielts_listening_test_list";
if ($id_test_filter) {
...
<head>
    <meta charset="UTF-8">
    <title>List các đề Ielts Listening Test</title>
...
<h1>List các đề Ielts Listening Test</h1>
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                foreach ($questions as $index => $question) {
                    $templatePart = 'database-listening-part-' . ($index + 1) . '';
                    echo "<a href='http://localhost/wordpress/contents/themes/tutorstarter/template/ieltslisteningtest/$templatePart/database-question-content-sample.php?id_part_filter=$question' target='_blank'>$question</a>";
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="editForm">
                    ...
                    Question Choice:<br>
                    (Mẫu:   listening part 1:_, listening part 2:_, listening part 3:_      ) 
...
function openEditModal(number) {
    $.ajax({
        url: 'http://localhost/wordpress/contents/themes/tutorstarter/template/ieltslisteningtest/test-list/get_question.php', // Fetch the question details
        type: 'POST',
        data: { number: number },
...
function saveEdit() {
    $.ajax({
        url: 'http://localhost/wordpress/contents/themes/tutorstarter/template/ieltslisteningtest/test-list/update_question.php',
        type: 'POST',
...
function saveNew() {
    $.ajax({
        url: 'http://localhost/wordpress/contents/themes/tutorstarter/template/ieltslisteningtest/test-list/add_question.php',
        type: 'POST',
...
function deleteRecord(number) {
    if (confirm('Are you sure you want to delete this question?')) {
        $.ajax({
            url: 'http://localhost/wordpress/contents/themes/tutorstarter/template/ieltslisteningtest/test-list/delete_question.php',
            type: 'POST',
            data: { number: number },
            success: function(response) {
