<?php
function get_problem_detail(WP_REST_Request $request) {
    global $wpdb;
...
/*
    ...
    if (isset($problem['similar_questions'])) {
        $similar_questions = json_decode($problem['similar_questions'], true);
        $problem['similar_questions'] = $similar_questions ? $similar_questions : array();
    }

    if (isset($problem['test_cases'])) {
        $test_cases = json_decode($problem['test_cases'], true);
        $problem['test_cases'] = $test_cases ? $test_cases : array();
    }*/
