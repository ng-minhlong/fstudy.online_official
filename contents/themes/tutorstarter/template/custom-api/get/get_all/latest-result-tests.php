...
function get_latest_user_results(WP_REST_Request $request) {
    ...
    foreach ($tables as $table_info) {
        $query = $wpdb->prepare(
            "SELECT dateform, testname, idtest, testsavenumber, {$table_info['result_column']} AS result 
             ...
             ORDER BY dateform DESC LIMIT 5", 
            $current_username
        );
        ...
        foreach ($results as $row) {
            $all_results[] = [
                ...
                'type' => $table_info['type'] // Thêm loại đề vào kết quả
            ];
        }
    }

    // Sắp xếp theo ngày làm bài, chỉ lấy 5 bài mới nhất
