...
function updateRealTimeTable(questionNumber, selectedOptions, completionValue) {
    ...
    if (questionCell) {
        ...
        if (selectedOptions.length > 0 || completionValue) {
            questionCell.innerHTML = `${selectedOptions.length > 0 ? selectedOptions.join(', ') : completionValue}`;
...
function initializeQuickViewTable(quizData) {
    ...
    moduleMap.forEach((questions, module) => {
        ...
        questions.forEach((question, index) => {
            tableHtml += `<td id="question-cell-${question}"></td>`;
...
function CheckSubmit() {
    ...
    for (let questionNumber = 1; questionNumber <= quizData.questions.length; questionNumber++) {
        ...
        const questionIDs = question.id_question;
        const domainQuestion = question.category;
        const typeQuestion = question.type;
        let typeModule = question.question_category;

        // Chuyển đổi typeModule
        if (typeModule === "Section 1: Reading And Writing") {
            typeModule = "verbal_1";
        } else if (typeModule === "Section 2: Reading And Writing") {
            typeModule = "verbal_2";
        } else if (typeModule === "Section 1: Math") {
            typeModule = "math_1";
        } else if (typeModule === "Section 2: Math") {
            typeModule = "math_2";
        } else {
            typeModule = "undentified";
        }
        ...
        if (questionCell) {
            userAnswer = questionCell.textContent.trim();
        }

        if (!isFullTest) {
            resultJson[`Question ${questionNumber}`] = {
                id: questionIDs,
    ...
    // Hiển thị ra giao diện nếu cần
    const userAnswerdiv = document.getElementById('useranswerdiv');
    if (userAnswerdiv) {
        userAnswerdiv.innerHTML += `<pre>${JSON.stringify(resultJson, null, 2)}</pre>`;
    }

    const resultsDiv = document.getElementById('results-output');
    if (resultsDiv) {
        resultsDiv.innerHTML = `<pre>${JSON.stringify(resultJson, null, 2)}</pre>`;
    }
}
